...
| Course finder | All | Three questions instead of a filter grid: who the student is, where and what, and what matters to the family. The description can be typed in the counsellor's own words and is read into the answers by the portal's own rules, with the AI, where the team has switched it on, only picking from the lists the portal supplies; a CGPA is never turned into a percentage and no figure is ever read out of the AI. The matches come back in order, grouped as a strong match, worth a look or a stretch, each with the reasons it is there and the cautions against it. A program up to a quarter over the budget is shown and marked, never dropped in silence. Shortlist, compare or hand the same filters to search. |
