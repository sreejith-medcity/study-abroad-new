  ...
  return (
    <Link
      ...
      className={cn(
        ...
        isActive ? "bg-brand-50 text-brand-700 ring-1 ring-inset ring-brand-100" : "text-ink-soft hover:bg-surface-2 hover:text-ink",
      )}
    >
...
function Groups({ groups, onNavigate }: { groups: NavGroup[]; onNavigate?: () => void }) {
  ...
  return (
    <nav aria-label="Main" className="space-y-5 p-3">
      {groups.map((group) => (
        <div key={group.title}>
          <p className="px-2.5 pb-1.5 text-[10px] font-semibold uppercase tracking-[0.16em] text-muted/70">{group.title}</p>
