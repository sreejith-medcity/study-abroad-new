...
export async function moveCommissionAction(_: FormState, formData: FormData): Promise<FormState> {
  ...
  if (commission.status === d.status) return { error: `Already ${d.status.toLowerCase()}.` };
  ...
  if (d.status === "SETTLED" && !partnerInr) {
    return { error: "Enter the rupee amount before paying the partner.", fieldErrors: { partnerAmountInr: ["Required"] } };
  }
